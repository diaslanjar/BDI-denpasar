    ...
    <header class="masthead">
        <div class="container px-4 px-lg-5 h-100">
            <div class="row gx-4 gx-lg-5 h-100 align-items-center justify-content-center">
                <h2 class="text-white font-weight-bold text-center"><br>Ubah Nama Peserta</h2>

                <form action="<?php echo base_url('penugasan/update/' . $data['id']) ?>" method="POST">
                    <div class="form-group">
                        <label class="text-white">Lokasi Unit</label>
                        <select name="kode_unit" id="kode_unit" class="form-control">
                            <option value="">Pilih Unit</option>
                            <?php  
                                foreach ($unit as $i => $d) {
                                    ?>
                                    <option <?= ($d['id'] == $data['unit_id']) ? 'selected' : '' ;?> value="<?= $d['id']?>">Kode: <?= $d['kode_unit']?> | Alamat: <?= $d['alamat_unit']?></option>
                                    <?php 
                                }
                            ?>
                        </select>
                    ...
                    <div class="form-group text-center">
                        <a href="/user/peserta" class="btn btn-md float-left btn-danger">Kembali</a>
